...
object ProguardPlugin extends Plugin {
  ...
  val makeJarFilter = SettingKey[Tuple2[String => String, String => String]]("makeJarFilter", "Filters for Injars and OutJars respectively")
  ...
  def proguardArgsTask: Initialize[Task[List[String]]] = {
    (proguardLibraryJars, proguardInJarsTask, artifactPath in (Compile, packageBin), makeJarFilter, minJarPath, proguardDefaultArgs, proguardOptions, packageBin in Compile, streams) map {
      (plj, pij, jp, makejf, mjp, pda, po, pb, s) =>
